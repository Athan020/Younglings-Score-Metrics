...
export class DatabaseService {
  ...
  poComments(comment, uid) {
    ...
    teams.subscribe(response => {
      response.map(element => {
        setTimeout(() => {
          if (element['pos'].includes(uid) && flag) {
            this.afStore.collection('sprints').doc(element['name'] + '-' + element['totalSprints']).update({ poComment: comment });
            flag = false;
          }
        }, 1000);
  ...
  updateRatings(score, sprintNum) {
    const team: Observable<any> = this.afStore
      .collection('teams')
      .snapshotChanges()
      .map(actions => {
        return actions.map(a => {
      ...
      response.map(element => {
        setTimeout(() => {
          if (element.pos.includes(this.afAuth.auth.currentUser.uid) && flag) {
            const sprint = this.afStore.collection('sprints').doc(sprintNum).valueChanges();
            sprint.subscribe(res => {
              const tempScore = res['score'] + score;
              const newRatings = res['ratingsReceived'] + 1;
              this.afStore.collection('sprints').doc(sprintNum).update({ score: tempScore, ratingsReceived: newRatings });
              flag = false;
            });
          }
    ...
    users.subscribe(r => {
      r.map(e => {
        if (e.user === this.afAuth.auth.currentUser.uid && userFlag) {
          const ratedTeams = e.teamsRated;
          ratedTeams.push(element.name);
          this.afStore.collection('users').doc(e.id).update({ teamsRated: ratedTeams });
          userFlag = false;
    ...
    teams.subscribe(response => {
      response.map(element => {
        setTimeout(() => {
          ...
          //  console.log(element);
          if (element.pos.includes(this.afAuth.auth.currentUser.uid) && flag) {
            if (element.id === this.afStore.collection('sprints').doc(element['name'] + '-' + element['totalSprints'])) {
